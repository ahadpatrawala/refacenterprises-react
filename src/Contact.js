...
const API_PATH = 'https://www.refacenterprises.com/contactus.php';
		...
		axios({
			...
			.then(result => {
			  ...
			  if (result) {
				...
				window.location.href ="https://www.refacenterprises.com/message";
			  } else {
				this.setState({ error: true });
				window.location.href ="https://www.refacenterprises.com/error";
			  }
			})
			.catch(error => {
...
render(){
    ...
    <div className="clearfix"></div>
		<div className="container">
		 <form id="form" className="contact_form" action="#" method="POST">
				<div className="message">
					<div className="col-md-6 col-sm-6 grid_6 c1">
						<input type="text" id="name" name="name" className="text" placeholder="Name" value={this.state.name} onChange={this.handleChange} required pattern="^[a-zA-Z ]+$"/>
						<input type="email" id="email" name="email" className="text" placeholder="Email" value={this.state.email} onChange={this.handleChange} required pattern="\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*"/>
						<input type="tel" id="phone" name="phone" className="text" placeholder="Phone" required maxLength="10" value={this.state.phone} onChange={this.handleChange} pattern="^\d+$"/>
					</div>
					<div className="col-md-6 col-sm-6 grid_6 c1">
						<textarea id="body" placeholder="Message" name="body" value={this.state.body} onChange={this.handleChange} required></textarea>
					</div>
					<div className="clearfix"> </div> 
				
				</div>

                        <input type="submit" className="more_btn" value="Submit" name="Submit" onClick={this.handleSubmit}/>
		    </form>
                </div>
	</div>
	<div className="map">
